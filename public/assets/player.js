...
function RunPlayer(strVideoPlayerID, intVideoWidth, intVideoHeight, strServerIP, intServerPort, boolSecureWebsocket, strOrArrayLiveAlias, strSecureToken, boolAutoPlay, boolShowControls, intLocalSeekMinutes, strPosterURL, boolBigButtons) {
    ...
    if (detectmob()) {
        ...
        video.autoplay = true;
        mobileBrowser = true;
        setTimeout(function () { videoControls.style.display = "none"; volSlider.style.display = "none"; }, 1000);
    }
    ...
    function AttachGUIControls() {
        ...
        var changePlayPauseState = function () {
            if (video.paused) {
                ...
                setTimeout(function () { videoControls.style.display = "none"; volSlider.style.display = "none"; }, 3000);
            }
        }
        ...
        if (!boolShowControls) {
            ...
            video.addEventListener('mousemove', function () {
                videoControls.style.display = "block";
                clearTimeout(timeout);
                timeout = setTimeout(function () { videoControls.style.display = "none" }, 3000);
            });
        }
        ...
        playpause.addEventListener('click', function (e) {
            if (!playerStarted) {
                video.poster = '/assets/video/bg4.gif';
                PlayStream();
            }
            else {
                if (video.paused) {
    ...
    function EnlargeControls() {
        ...
        volume.style.backgroundSize = "28px 60px";
    ...
    function RunWebSocket() {
        ...
        ws.onmessage = function (evt) {
            ...
            if (byteView[0] == 1)  //Init segment
            {
                ...
                checkPauseStream = checkPauseStream + 1;
                if (checkPauseStream == 3) {
                    closePlayback();
                    PlayStream();
                    checkPauseStream = 1;
                }
            ...
            if (playOK && startupDone && rendererStarted && !realTimeStream && !timeshiftSeeking && !timeshiftStartingAfterSeek)   //Perform buffering if needed
            {
                ...
                else if ((video.duration - video.currentTime >= 1) && bufferingInProgress) {
