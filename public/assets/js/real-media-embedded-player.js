// ✅ Export a factory function so external code can use it
var streamUrl_FromFile = '';
var appName_FromFile = '';
var streamId_FromFile = '';
var token_FromFile = '';

// Suppress VideoJS deprecation warnings for external plugins
if (typeof window !== 'undefined' && window.console) {
  const originalWarn = console.warn;
  console.warn = function(...args) {
    const message = args.join(' ');
    // Filter out known VideoJS plugin warnings that we can't control
    if (message.includes('videojs.mergeOptions is deprecated') ||
        message.includes('Using the tech directly can be dangerous') ||
        message.includes('VIDEOJS: WARN: Using the tech directly')) {
...
export function createWebPlayer(config) {
  streamUrl_FromFile = config.streamUrl;
  appName_FromFile = config.appName;
  streamId_FromFile = config.streamId;
  token_FromFile = config.token;
}
window.createWebPlayer = createWebPlayer;
...
class WebPlayer {
  constructor(configOrWindow, containerElement, placeHolderElement) {
    ...
    */
    ...
    this.setDefaults();
    this.streamId = streamId_FromFile;
  ...
  initialize() {
    return this.loadVideoJSComponents().then(() => {
      ...
      if (this.is360 && !window.AFRAME) {
        return import('./real-media-aframe-master-8ee6af74-c7iHvFLi.js').then(function (n) {
          return n.a;
        });
      }
    }).catch(e => {
  ...
  loadDashScript() {
    if (this.playOrder.includes("dash") && !this.dashjsLoaded) {
      return import('./real-media-dash.all.min-af8b7133-1wUNRr-i.js').then(function (n) {
        return n.d;
      }).then(dashjs => {
  ...
  setDefaults() {
    ...
    this.placeHolderElementInitialDisplay = "flex";
  ...
  loadWebRTCComponents() {
    if (this.playOrder.includes("webrtc")) {
      return import('./real-media-videojs-webrtc-plugin-5fadffd3-Lz5nGVSV.js').then(css => {
        ...
        return import('./real-media-videojs-webrtc-plugin.es-f339fb8c-n6SxyHZa.js').then(videojsWebrtcPluginLocal => {
  ...
  loadVideoJSComponents() {
    if (this.playOrder.includes("hls") || this.playOrder.includes("ll-hls") || this.playOrder.includes("vod") || this.playOrder.includes("webrtc")) {
      ...
      if (!this.videojsLoaded) {
        return import('./real-media-video-js.min-9b374ff4-kk4alGLA.js').then(css => {
          var styleElement = this.dom.createElement('style');
          styleElement.textContent = css.default.toString(); // Assuming css module exports a string
          this.dom.head.appendChild(styleElement);
        }).then(() => {
          return import('./real-media-video.es-0951ae41-boIeAy0a.js').then(function (n) {
            return n.a;
          });
        }).then(videojs => {
          ...
          return import('./real-media-plugin-06260ef3-rhOmlHFH.js');
        }).then(() => {
  ...
  playWithVideoJS(streamUrl, extension) {
    ...
    this.videojsPlayer = videojs(this.videoPlayerId, {
      poster: "./assets/video/loading.gif",
    ...
    if (extension == "m3u8") {
      this.videojsPlayer.on('xhr-hooks-ready', () => {
        this.videojsPlayer.ready(() => {
          // Use proper VideoJS API instead of directly accessing tech
          try {
            const tech = this.videojsPlayer.tech(true);
            if (tech && tech.vhs && tech.vhs.xhr) {
              tech.vhs.xhr.onRequest(this.insertSecurityParameters);
            }
          } catch (error) {
            Logger_1.warn("Could not set up HLS security parameters: " + error);
          }
        });
      });
      this.videojsPlayer.ready(() => {
    ...
    if (this.autoPlay) {
      ...
      this.videojsPlayer.play().catch(e => {
        ...
        if (e.name === "NotAllowedError" && !this.forcePlayWithAudio) {
          Logger_1.info("Autoplay blocked by browser policy, trying with muted audio");
          this.videojsPlayer.muted(true);
          this.videojsPlayer.play().catch(muteError => {
            Logger_1.warn("Failed to play even with muted audio: " + muteError);
          });
        } else {
  ...
  getWebsocketURLForStream(streamIdToPlay) {
    // this.websocketBaseURL = 'ws://185.185.51.140:5080/WebRTCAppEE/';
    this.websocketBaseURL = streamUrl_FromFile + appName_FromFile;
    return this.websocketBaseURL + streamId_FromFile + ".webrtc";
  }
...
document.getElementById("unmuteButton").addEventListener("click", function () {
  ...
  if (webPlayer.isMuted()) {
    webPlayer.mutePlayer(false);
    button.innerHTML = '<img src="./assets/video/VolumeOn.svg" alt="Volume On" style="width: 4vh;height: 4vh;" />';
  } else {
    webPlayer.mutePlayer(true);
    button.innerHTML = '<img src="./assets/video/VolumeOff.svg" alt="Volume Off" style="width: 4vh;height: 4vh;" />';
  }
});

setTimeout(() => {

  document.getElementById("video-player_html5_api").addEventListener("click", function () {
    const video = document.getElementById('video-player_html5_api');
    const playBtn = document.getElementById('play-video');

    function pauseVideoHandler(e) {
      console.log('video click or touch');
      if (!video.paused) {
        video.pause();
        playBtn.style.display = 'block';
      }
      // Prevents the click event after touchstart
      e.preventDefault();
    }
    ...
    // For video pause
    video.addEventListener('click', pauseVideoHandler);
    video.addEventListener('touchstart', pauseVideoHandler);

    // For play button
    playBtn.addEventListener('click', playVideoHandler);
    playBtn.addEventListener('touchstart', playVideoHandler);

    // // Optionally, hide play button whenever video starts playing (in case of big play button, auto start, etc.)
    // video.addEventListener('play', function () {
    //   playBtn.style.display = 'none';
    // });


    // const pipBtn = document.getElementById('pictureIn');

    // pipBtn.addEventListener('click', async function () {
    //   try {
    //     // If already in PiP, exit it; else enter PiP
    ...
    //       await document.exitPictureInPicture();
    //     } else {
    //       // Ensure video is playing to unlock PiP on some browsers
    //       if (video.paused) {
    //         await video.play();
    //       }
    //       await video.requestPictureInPicture();
    ...
    // pipBtn.addEventListener('touchstart', (e) => {
    //   e.preventDefault(); // Prevents firing 'click' event as well on some devices
    //   togglePiP();
    // });
...
window.destroyAndResetWebPlayer = function() {
  if (window.webPlayer) {
    // Destroy the current player instance
    window.webPlayer.destroy();
    
    // Clear any running timers
    if (window.webPlayer.tryNextTechTimer !== -1) {
      clearTimeout(window.webPlayer.tryNextTechTimer);
    }
    if (window.webPlayer.dashLatencyTimer) {
      clearInterval(window.webPlayer.dashLatencyTimer);
    }
    
    // Reset container visibility
    window.webPlayer.setPlayerVisible(false);
    
    // Clear the container content
    if (window.webPlayer.containerElement) {
      window.webPlayer.containerElement.innerHTML = '';
    }
    ...
    // Create a new WebPlayer instance with initial values
    window.webPlayer = new WebPlayer(window, document.getElementById("video_container"), document.getElementById("video_info"));
...
// Also expose the method through the webPlayer object for convenience
if (window.webPlayer) {
  window.webPlayer.destroyAndReset = window.destroyAndResetWebPlayer;
}
