...
export class MarketDetailComponent {
      ...
      .subscribe((changes:any) => {
        changes.forEach((change:any) => {
          ...
          if (change.type === 'added') {
            ...
            const index = this.AllMarketList.findIndex((obj:any) => obj.exMarketId === pt.exMarketId);

            if (index !== -1) {
              this.AllMarketList[index].oddsData.status = pt.oddsData.status;
              this.AllMarketList[index].oddsData.inPlay = pt.oddsData.inPlay;
              this.AllMarketList[index].oddsData.totalMatched = pt.oddsData.totalMatched;
              ...
              if (this.marketId == pt.exMarketId && (pt?.oddsData?.status !== 'ONLINE' && pt?.oddsData?.status !== 'OPEN')) {
                this.isBetsSlipOpened = '';
                this.isValueBetsSlip = 0;
  ...
  getBookmakerDataFirebase(projectDynamic: any) {
    ...
    this.bookmakerSubscription = projectDynamic.collection('Bookmakers', (ref:any) => {
      return ref
        ...
        changes.forEach((change:any) => {
          ...
          if (this.onOpenPage) {
            ...
            setTimeout(() => {
              ...
              const exMarketIds = new Set(this.firebaseResposeBookmaker.map((item: any) => item.exMarketId));
              this.AllMarketList = this.AllMarketList
                .filter((bookmakerMarket: any) => bookmakerMarket.oddsType === "MATCH_ODDS" ||
                  (bookmakerMarket.oddsType === "BOOKMAKERS" && exMarketIds.has(bookmakerMarket.exMarketId))
                )
      ...
      .subscribe((changes:any) => {
        changes.forEach((change:any) => {
          ...
          if (change.type === 'added') {
            ...
            let index = this.AllFancyMarkets.findIndex((obj:any) => obj.exMarketId === pt.exMarketId);
            if (index !== -1) {
              ...
              this.AllFancyMarkets[index].oddsData.totalMatched = pt.oddsData.totalMatched;
      ...
      .subscribe((changes:any) => {
        changes.forEach((change:any) => {
          ...
          if (this.onOpenPageSportbook) {
            ...
            setTimeout(() => {
              // Use a Set for faster lookup
              const exMarketIds = new Set(this.firebaseResposesportBook.map((item: any) => item.exMarketId));
              this.AllFancyMarkets = this.AllFancyMarkets
                .filter((sportbookMarket: any) => sportbookMarket.oddsType === "FANCY" ||
                  (sportbookMarket.oddsType === "SPORTSBOOK" && exMarketIds.has(sportbookMarket.exMarketId))
                )
                .sort((a: any, b: any) => a.sequence - b.sequence);
  ...
  getMatchedBetList() {
    ...
    this.backendService.getAllRecordsByPost(CONFIG.eventMatchedBetList, req).then((record:any) =>{
  ...
  changeFancyMarket(tableFlag: any, onChangeData?: any) {
    ...
    if(this.isDesktop){
      ...
      this.AllFancyMarketsSort = this.AllFancyMarketsFiltered.filter((market: any) => {
        if (market?.oddsData?.status != 'CLOSED' && market?.tableFlag !== 'SPORTSBOOK') {
          return market
        }
        else {
          return null
  ...
  changeOddsMarket(marketid: any, tableFlag: any, onChangeOddData?: any) {
    ...
    if (marketid == 'All') {
      ...
      const matchOdds = matchOddsData.filter((market: any) => market.marketType === 'MATCH_ODDS' );
      const bookmakers = matchOddsData.filter((market: any) =>  market.marketType === 'Bookmakers-2' && market.marketName.includes('Bookmaker'));
      const otherMarkets = matchOddsData.filter((market: any) => market.marketType !== 'MATCH_ODDS' && !market.marketName.includes('Bookmaker')).sort((a: any, b: any) =>  b.sequence - a.sequence);

      this.matchOddsDataUpdated = [...matchOdds, ...bookmakers, ...otherMarkets];
      // console.log(this.matchOddsDataUpdated)

    }
    else {
      this.matchOddsDataUpdated = this.AllMarketList.filter((market: any) => {
        if (market.exMarketId == marketid && tableFlag == market.marketType && market?.oddsData?.status != 'CLOSED') {
          return market
        }
        else {
          return null
      ...
      if (this.AllMarketList[i].exMarketId == exMarketId) {
        ...
        if (getPlFilter.length == 2 || getPlFilter.length == 0) {
          ...
          if (getFavPl > userProfitLoss[OthselectionData]) {
            ...
            cashOutStake = parseFloat(Number(subPl / ((getFavSelection["layOdds"] / 100) + 1)).toFixed(2));
            resPrice = getFavSelection["layOdds"];
            resSide = "LAY";

          } else {

            cashOutStake = parseFloat(Number(subPl / ((getFavSelection["backOdds"] / 100) + 1)).toFixed(2));
          ...
          if (getFavPl > 0) {
            cashOutStake = parseFloat(Number(getTotalPlSum / ((getFavSelection["layOdds"] / 100) + 1)).toFixed(2));
            resPrice = getFavSelection["layOdds"];
            resSide = "LAY";
          } else {
            cashOutStake = parseFloat(Number(getTotalPlSum / ((getFavSelection["backOdds"] / 100) + 1)).toFixed(2));
            resPrice = getFavSelection["backOdds"];
            resSide = "BACK";
          }
        }
